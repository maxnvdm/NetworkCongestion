...
class TCPTopo(Topo):
    ...
    def build(self, n=2):
        ...
        h2 = self.addHost('h2')
        self.addLink( h2, switch, bw=args.bw_net, delay=args.delay,
                          max_queue_size=args.maxq)
        return
...
def start_iperf(net):
    ...
    server1 = h1.popen("iperf -s -w 16m")
    ...
    client2 = h2.popen("iperf -c %s -t %d" % (h1.IP(), args.time + 5))
    client1 = h1.popen("iperf -c %s -t %d" % (h2.IP(), args.time + 5))
